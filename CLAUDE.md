...
| **11** | **Stress test & polish** | **DONE** — lance-test **x100: 100/100 ALL PASS** (all 5 tests 100%, single persistent daemon, report `tests/lance_logs/results_100x_20260609_143735.md`). Daemon adaptive backoff (idle CPU ~100%→~9%) + `--debug` flag. AddNetInterface A2065 → DHCP lease on real network. MAC-display cosmetic (issue #3) re-scoped won't-fix (lance-test binary). |
...
- **`cpu_berr_n` RE-SCOPED (2026-06-15) — N/A on doorbell, not a TODO.** The `$0000`/`cpu_berr_n` note was about the *old bridge* (`rtl/A2065/legacy/a2065_registers.v`: WATCHDOG_CYCLES=700000 → ST_BERR → `cpu_berr_n<=0`+`cpu_data_out<=$0000`, with `cpu_berr_n` never threaded to the CPU). The doorbell RTL (`a2065_regfile.v`/`a2065_ddram.v`) has **no watchdog and no berr**. Register reads are zero-latency combinational (`a2065_regfile.v:74`) → can't time out → no `$0000`-on-timeout. Only writes (`regs_nrdy`, `a2065_regfile.v:128`) + boardram (`a2065_bram_nrdy`, `a2065_ddram.v:144`) stretch DTACK (OR-tied `minimig.v:699`); with no watchdog a dead daemon *hangs* the bus, not `$0000`. **Real BERR is infeasible:** the 68020 runs on TG68 whose `berr` port is an Atari-ST 68000 dummy and isn't wired (`TG68K.vhd:37,83`); fx68k has `BERRn` but is 68000-only + hardwired `.BERRn(1)` (`cpu_wrapper.v:265`). If the daemon-crash hang ever needs hardening, use a *watchdog-release* (timeout in W_WAIT/NR_WAIT → deassert nrdy, return `$FFFF`) — core-agnostic, no BERR. Tracked as a future option only.
...
- **Issue A FIXED (2026-06-15) — on-wire station MAC now unique, NIC-derived.** Wire src MAC = `00:80:10:00:04:2B` (low bytes from eth1 `68:68:68:00:04:2B`); zero frames remain with `…00:00:00` (`test.pcap`). **Root cause:** the munge unit (`mac.cpp`) and `registers.cpp` keep *separate* `fakemac` copies. `dofakemac`/`mungepacket` match against `mac.cpp`'s copy, which was set once at startup from the NIC (`…04:2B`) and **never matched what the Amiga actually emits** — the autoconfig `er_SerialNumber` station address = `00:80:10:00:00:00` (FPGA-side, daemon-inaccessible; the daemon cannot change the Amiga-visible station MAC). So `fakemac (…04:2B) ≠ frame src (…00:00)` → swap never fired → wire stayed `…00:00`. `chip_init()` updated only `registers.cpp`'s copy from the init-block PADR, never the munge unit's. **The literal "override fakemac" idea is wrong** — `mac.cpp` `fakemac` must EQUAL the Amiga's on-wire address so munge can find/replace it; `realmac` carries the unique NIC bytes. **Fix (3 parts):** (1) `registers.cpp` `chip_init()` calls `mac_set_fakemac()` to push the Amiga's real PADR address into the munge unit so `fakemac → realmac` finally matches; (2) `rings.cpp` `gotfunc()` munges the RX frame **before** the accept filter (replies arrive addressed to `realmac`; filtering raw against `fakemac` would drop every unicast reply); (3) `main_doorbell.cpp` `daemon_set_default_mac(iface)` hardens `realmac`: selected iface → onboard eth0 (`ethernet_read_iface_mac`, unique/stable per board) → hostname/pid/time hash, never all-zero. Daemon-only, no FPGA rebuild. **Verified:** DHCP + ARP + ICMP echo req/reply all on `…04:2B`, bidirectional. See [[station-mac-ownership]] memory. (Amiga-*visible* station MAC stays `…00:00` — a true fix for that needs FPGA er_SerialNumber wiring, has autoconfig boot-race; cosmetic. MAC-*display* sign-extension issue #3 re-scoped won't-fix — lance-test binary, see Remaining Work.)
...
2. ~~**MAC display byte ordering**~~ — RE-SCOPED won't-fix (2026-06-15). lance-test shows `00:FFFFFF80:10:...` — the `0x80` byte sign-extends in a signed `char` printf inside the **pre-compiled `share:lance-test` binary** (no repo source). Display-only; wire MAC is correct (pcap-verified). Not fixable from this project.
...
4. ~~**Issue A — station MAC low bytes zero**~~ — DONE (2026-06-15). On-wire MAC now unique NIC-derived (`…04:2B`); daemon-only fix in `mac.cpp`/`registers.cpp`/`rings.cpp`/`main_doorbell.cpp`. See Known Issues / Notes + [[station-mac-ownership]].
5. ~~**Connect cpu_berr_n**~~ — RE-SCOPED, not a TODO. Premise was the old bridge; doorbell has no watchdog, reads are zero-latency (can't time out), and the TG68/020 has no usable BERR. See Known Issues / Notes. Future option: watchdog-release returning `$FFFF` (core-agnostic), not BERR.
